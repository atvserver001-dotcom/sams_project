...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (name !== undefined || description !== undefined || color_hex !== undefined) {
    ...
    const { error: updError } = await (supabaseAdmin.from('contents') as any).update(updateData).eq('id', id)
    if (updError) return NextResponse.json({ error: updError.message }, { status: 500 })
  }

  // 2. 디바이스 관계 수정 (전체 삭제 후 재삽입)
  if (device_ids) {
    const { error: delError } = await (supabaseAdmin.from('content_devices') as any).delete().eq('content_id', id)
    if (delError) return NextResponse.json({ error: delError.message }, { status: 500 })

    if (device_ids.length > 0) {
      const relations = device_ids.map((devId) => ({ content_id: id, device_id: devId }))
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params
  const { error } = await (supabaseAdmin.from('contents') as any).delete().eq('id', id)
