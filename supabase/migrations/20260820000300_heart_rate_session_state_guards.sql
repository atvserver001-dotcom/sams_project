-- Make stop/finalize lifecycle transitions retry-safe without changing prior migrations.

create or replace function public.stop_heart_rate_session(
  p_school_id uuid,
  p_session_id uuid,
  p_points jsonb,
...
begin
  ...
  -- A retried stop must be a pure read. In particular, altered retry payloads
  -- must not revise measurements or transport counters after the first stop.
  if v_session.status in ('awaiting_decision', 'completed') then
...
begin
  ...
  if v_result_count = 0 then
    raise exception using errcode = '55000', message = 'heart_rate_session_has_no_results';
  end if;
